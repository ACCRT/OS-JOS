...
extern void sbrk_alloc(struct Env *e, void *va, size_t len);
...
sys_page_alloc(envid_t envid, void *va, int perm)
{
	...
	//	-E_INVAL if va >= UTOP, or va is not page-aligned.
	//	-E_INVAL if perm is inappropriate (see above).
	if (va >= (void*)UTOP || (perm & (PTE_U | PTE_P)) != (PTE_U | PTE_P) || PGOFF(va) != 0 || (perm & (~PTE_SYSCALL)) != 0)
		return -E_INVAL;
	...
	//	-E_NO_MEM if there's no memoryto allocate any necessary page tables
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	if(envid2env(srcenvid, &s_env, 1) < 0 || envid2env(dstenvid, &d_env, 1) < 0) return -E_BAD_ENV;
	...
	if(page_insert(d_env->env_pgdir, p, dstva, perm) < 0) return -E_NO_MEM;
...
sys_page_unmap(envid_t envid, void *va)
{
	...
	if (va >= (void*)UTOP || ROUNDUP(va, PGSIZE) != va) return -E_INVAL;
...
sys_sbrk(uint32_t inc)
{
	...
	// region_alloc 为 static ，所以用sbrk_alloc包装，并extern
	sbrk_alloc(curenv, (void*)(curenv->va_start + curenv->va_len), inc);
...
my_syscall(struct Trapframe *tf){
	...
	curenv->env_tf.tf_regs.reg_eax = tf->tf_regs.reg_eax=syscall(tf->tf_regs.reg_eax,
							tf->tf_regs.reg_edx,
							tf->tf_regs.reg_ecx,
							tf->tf_regs.reg_ebx,
