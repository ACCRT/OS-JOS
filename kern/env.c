...
env_setup_vm(struct Env *e)
{
	...
	// Do NOT (yet) map anything into the user portion
	// UTOP之下为USER自由管理，但UTOP之上是只读的KERNEL映射，或者不可读写的KERNEL，所以从UTOP开始映射
	
	// NPDENTRIES number of page directory entries #define NPDENTRIES 1024 in mmu.h
	// UTOP USER TOP
	// PDX(UTOP) page directory index of UTOP
	// env_pgdir[PDX(UTOP)] get page directory index of UTOP, then find the content of this index in env_pgdir array
...
region_alloc(struct Env *e, void *va, size_t len)
{
	...
	// ROUNDDOWN(a,b) round down a by b e.g ROUNDDOWN(1024,1023) == 1023 ROUNDDOWN(1024,1024) ==1024
	// ROUNDUP(a,b) round up a by b e.g ROUNDUP(1023,1024) == 1024 ROUNDUP(1024,1024) ==1024
	...
	e->va_len = (uint32_t)(ROUNDUP(va+len, PGSIZE) - ROUNDDOWN(va, PGSIZE));
}
...
load_icode(struct Env *e, uint8_t *binary, size_t size)
{
	...
	// LAB 3: Your code here.
	// 把ELF中的code复制到p_va中，并通过映射p_va在env_pgdir中映射该code
	// 即把ELF中的code映射到env_pgdir中
	// 最后为stack开出内存页表
	...
	// e_phnum: program header number of envioronment
	// e_phoff: start of the program header offset from elf header
	...
	for (; ph < eph; ph++)
		...
		{
			// set p_va to env_pgdir(so we can get p_pa) so that after we can copy to p_va, it can be found in env_pgdir
			region_alloc(e, (void*)ph->p_va, ph->p_memsz);  
			// copy: ph->p_va = binary + ph->p_offset, copy_size: ph->p_filesz
			int i;
	...
	// env_tf: envrionment's saved registers
	...
	// switch cr3(base of physical address) again to kern_pgdir
