...
mem_init(void)
{
	...
	// Permissions: kernel RW, user NONE
	// Your code goes here:
	boot_map_region(kern_pgdir, KERNBASE, ROUNDUP((uint32_t)0xFFFFFFFF-KERNBASE, PGSIZE), 0, PTE_W | PTE_P);
...
mem_init_mp(void)
{
	...
	// 由于KSTKGAP没有设置权限，所以是0,只要访问到了，就是PGFALT
	int i;
	for (i = 0; i < NCPU; i++)
...
page_init(void)
{
	...
	for (i = npages-1; i > -1; i--) {
		// I/O空洞（640K之后的384K）与第0个page 与 代表MPENTRY_PADDR的page 不能被分配出去
		// MPENTRY_PADDR放的是AP的entry
		if((PGSIZE * i >= IOPHYSMEM && PGSIZE * i < EXTPHYSMEM) || i == 0 || i == MPENTRY_PADDR/PGSIZE)
			continue;
		// base menmory（0—640K，但不包括第一个4K） && pages,envs之后 是free的
...
page_alloc(int alloc_flags)
{
	...
	if(page_free_list)
	{
		// 把当前page_free_list的第一个Page分配出去，再把page_free_list指向下一个
		struct Page* tmp = page_free_list;
		page_free_list = page_free_list -> pp_link;
		// 把该地址转换成kernel virtiual address，利用memset把这个page都设为0
		if(alloc_flags & ALLOC_ZERO) memset(page2kva(tmp), '\0', PGSIZE);
		tmp->pp_link = NULL;
		return tmp;
	}
	return 0;
...
page_alloc_npages(int alloc_flags, int n)
{
	...
	struct Page* now = NULL;
	struct Page* tmp_before = NULL;
	...
	if(chunk_list)
	{
		...
		while(now && i<=n)
		{
			...
			if(i == n)
			{
				...
				now->pp_link = NULL;
				if(alloc_flags & ALLOC_ZERO) memset(page2kva(tmp), '\0', n * PGSIZE);
				return tmp;
			}
			if(now->pp_link == NULL)
				break;
...
page_free_npages(struct Page *pp, int n)
{
	...
	for(i = 0;i < n-1; i++)
	{
		if(tmp == NULL || tmp->pp_link == NULL || page2pa(tmp->pp_link)-page2pa(tmp) != PGSIZE)
			return -1;
		tmp = tmp->pp_link;
	}
	...
	else
	{
		...
		while(chunk_now->pp_link && page2pa(chunk_now->pp_link) < page2pa(pp))
...
page_free(struct Page *pp)
{
	...
	{
		while(tmp->pp_link && page2pa(tmp->pp_link) < page2pa(pp))
			tmp = tmp->pp_link;
...
page_remove(pde_t *pgdir, void *va)
{
    ...
    // the standard method is to set *tmp = *tmp & !PTE_P
    *tmp = 0;
    tlb_invalidate(pgdir,va);
}
