...
void ring0_call(void (*fun_ptr)(void)) {
	...
	// 2. Map GDT in user space (sys_map_kernel_page)
	// User applications in JOS could map any kernel page into userspase using sys_map_kernel_page
	int t = sys_map_kernel_page((void* )gdtd.pd_base, (void* )vaddr);
	if (t < 0) cprintf("ring0_call: sys_map_kernel_page failed, %e\n", t);
